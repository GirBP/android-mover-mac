...
# SwiftPM-ресурси AndroidMover-таргету (Resources/Localizable.xcstrings — String Catalog)
# пакуються SPM в окремий .bundle поруч із бінарником у .build/, не всередину нього — без
...
# Версія — з git-тега: CFBundleShortVersionString з останнього тега (без "v"),
...
# Smoke-тест — без ресурсного бандла вище (Bundle.module) додаток крешив би одразу при
...
# Після kill додатка (SIGTERM) дочірній adb-процес (DeviceStore track-devices) не має
...
# Перевіряємо лише дітей власного smoke-процесу (pgrep -P), а не будь-який `adb
# track-devices` у системі — інакше smoke міг би сплутати з сиротою робочий екземпляр
# додатка, що працює поруч, паралельно з тестом.
