...
`scripts/e2e_device.sh` (0.2, спринт 0):
...
2. Ганяє РІВНО ТІ Ж shell-скрипти, що складає сам `ADBClient` (дослівно скопійовані в
...
**`md5sum.txt`:** checksum-верифікація реалізована у v0.11.0 (`ADBClient.checksums`), але golden-крок
для неї ще не доданий. З'явиться разом з `amctl capture`, який
замінить ручне копіювання скриптів у цей bash.
...
## v0.15.0: `amctl capture`

З M3 транскрипти пише сам застосунковий будівник argv (`RecordingTransport`), без ручного
