...
/// Батчевий шлях для плоских файлів (не тек, не symlink) — переносить сотні-тисячі файлів
/// без окремого `find`/`pull`/`rm` на кожен: підрахунок для файлів бере розмір з лістингу
/// (уже відомий), pull — один процес на ≤ADBClient.batchSize файлів, rm — один shell на батч.
/// Інваріанти незмінні: кожен файл окремо проходить дати → verify → атомарний move; видалення
/// з телефона — лише після verify саме цього файла; провал батчевого pull, відсутній чи
/// битий файл → поштучний transferOne (з resume після обриву).
extension TransferEngine {
    static func isPlainFile(_ entry: RemoteEntry) -> Bool {
        !entry.isDirectory && !entry.isSymlink
    }

    /// Видалення верифікованої теки пофайлово: батчі `rm` на файли з очікуваної мапи (усі
    /// щойно звірені з копією), потім `rmdir` лише порожніх тек знизу вгору. Чужий файл, що
    /// з'явився на телефоні під час переносу, і його тека — лишаються (повертаються).
    ...
    func transferFileBatch(
        ...
        // md5 усіх файлів батчу одним викликом (замість одного на файл).
