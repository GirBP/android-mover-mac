...
/// internal (не private) — потрібен і тут (run()), і в ProcessRunner+Stream.swift (stream()).
...
public enum ProcessRunner {
    ...
    /// `fileSystemRepresentation`/`URL`/`FileManager`, тому жодної unicode-нормалізації.
    /// `Foundation.Process` на Darwin для `arguments`/`environment` внутрішньо йде саме через
    /// filesystem-representation і мовчки НФД-декомпонує канонічно-композиційні символи в
    /// дочірньому процесі (підтверджено ізольовано: `Process(arguments: ["й"])` дитина бачить
    /// "и"+U+0306, не "й"). `Array(string.utf8)` (як і `String.withCString`, використаний нижче
    /// для самого шляху виконуваного файла) віддає рівно ті байти, що зберігає Swift String —
    /// ніякої decomposition-логіки тут узагалі немає.
    ...
    /// Спороджений процес одразу після успішного `posix_spawn`: `ChildProcess`-дескриптор і
    /// живі `FileHandle`-и наших (батьківських) read-кінців обох пайпів stdout/stderr.
    /// internal (не private) — повертається з spawnChild() і споживається і тут (run()),
    ...
    /// Спільний «хребет» `posix_spawn` — той самий код (argv/envp байт-у-байт, пайпи,
    ...
    static func spawnChild(
        ...
        // Оточення: базою завжди служить ProcessInfo.processInfo.environment (сама по собі
        // байт-у-байт з реального environ — перевірено ізольовано, вона не йде через
        // filesystem-representation), зверху — override, якщо є.
        ...
        // Реєстр-бекстоп (ChildProcessRegistry.swift) — єдина точка спавну і для run(), і
    ...
    /// Викликається з App-таргету (AppDelegate: SIGTERM/SIGINT-обробник,
    /// `applicationWillTerminate`) — ескалює (SIGTERM→3с→SIGKILL) усі ще живі дочірні процеси,
    /// попри те, що сам App-таргет не тримає жодного `ChildProcess` напряму (той живе лише
            ...
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                if let timeout {
                    ...
                    source.setEventHandler {
                        ...
                        // Перевіряємо isRunning першим, до ескалації. Якщо дитина вже
                        // завершилась (напр. рівно на дедлайні) — це не таймаут: позначення
                        // timedOut=true до перевірки isRunning хибно перетворило б вдалий
                        // прогін, що завершився саме в цю мить, на ADBError.timeout.
                        guard child.isRunning else { return }
                        guard timedOut.trySet() else { return }
                        // Одноразова ескалація (SIGTERM→3с→SIGKILL) живе на самому
                ...
                // spawnChild() вище, аж до кінця цієї функції: .readabilityHandler не тримає
                // FileHandle живим сам по собі — локальна змінна, звільнена ARC до EOF, знімає
                // dispatch-source передчасно, і group.leave() для цього пайпа ніколи не
                // викликається — group.wait() нижче зависає назавжди.
                ...
                // пайпів, які крутяться на власних internal dispatch-чергах).
                var status: Int32 = 0
