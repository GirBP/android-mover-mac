...
/// Журнал незавершених операцій — на відміну від HistoryStore (що сталось), це «що мало
/// статись»: запис створюється при постановці в чергу, стан кожного елемента дописується
/// щойно рушій його закінчив, запис закривається по завершенню. Крах/kill/вимкнення живлення
/// посеред операції лишає відкритий запис — наступний запуск пропонує продовжити (перенести
/// решту; довидалити «скопійовано, але не видалено» після повторної перевірки копії).
...
public struct JournalRecord: Codable, Sendable, Identifiable, Equatable {
    ...
    /// Стабільна ідентичність телефона (`DeviceIdentity.stableID`) — відновлення шукає
    /// пристрій за нею, а `serial` лишається лише міткою транспорту на момент старту. Старі
    /// записи без поля декодуються з nil (резолв лише за тим самим serial).
