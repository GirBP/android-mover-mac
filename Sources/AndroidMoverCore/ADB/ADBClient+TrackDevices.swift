...
/// 2.4: `adb track-devices` (стрім змін підключення) + `wait-for-device` (B2) — усе, що
/// стосується ПРИСУТНОСТІ пристрою, а не вмісту файлової системи на ньому.
extension ADBClient {
    // MARK: - 2.4: adb track-devices (стрім змін підключення)
    ...
    /// довантажувати `devices -l` окремо НЕ треба (за задумом 2.4 — рішення UI-кодера, коли
    /// саме довантажувати назву моделі для показу).
    ...
    ///
    /// 2.4-фікс: некоректний (не-hex) префікс довжини БІЛЬШЕ не зупиняє розбір на місці —
    /// раніше `break` лишав "поламаний" байт на початку remainder НАЗАВЖДИ (наступний виклик
    /// бачив той самий невалідний префікс знову, кадр ніколи не розбирався, буфер ріс без
    /// обмежень з кожним новим чанком). Тепер відкидаємо РІВНО 1 байт і продовжуємо пошук
    /// валідного кадру з наступної позиції — ресинхронізація, а не глухий кут. Запобіжник:
    /// якщо після повного проходу лишається >64 КБ, з яких не вдалось розібрати ЖОДНОГО кадру
    /// (чужорідний потік, що ніколи не резинхронізується — напр. валідний на вигляд, але
    /// величезний hex-префікс, що заявляє кадр більший за все, що коли-небудь прийде) —
    /// буфер очищується цілком, а не росте вічно.
