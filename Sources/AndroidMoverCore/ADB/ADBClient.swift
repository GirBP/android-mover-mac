...
///
/// Клас розбито на кілька файлів, щоб жоден не переростав ~400 рядків: цей файл — ядро
/// (властивості/init/discover/run/devices); ADBClient+TrackDevices.swift — `adb
/// track-devices` + waitForDevice; ADBClient+Listing.swift — лістинг теки і рекурсивний
/// перелік файлів/тек (без -type — статMTimes лишається тут же); ADBClient+FileOps.swift —
/// мутуючі операції (delete/mkdir/move/rename/push/remoteExists/storage/rescan). Усе — методи
/// одного типу через `extension`, розподілені по файлах лише для читабельності;
/// `private`/`internal` розставлено так, щоб extension у іншому файлі бачив усе, що йому
/// потрібно (Swift `private` — лише для same-file extensions того самого типу).
public final class ADBClient: @unchecked Sendable {
    ...
    // Усі таймаути нижче — idle-таймаути (ProcessRunner.run), не загальна тривалість
    ...
    /// Скільки чекати на повернення пристрою після обриву — довше за звичайні операції:
    ...
    /// Тестовий гачок — підміняє `Self.findTimeout` для цього клієнта (recursiveFiles/
