...
/// (для підрахунку прогресу й верифікації), плюс `pull` (копіювання з телефона). Скрипти
/// беруться зі словника `ADBScripts`; тут — лише виклик і розбір виводу.
extension ADBClient {
    // MARK: - Список теки
    ...
    ///
    /// Без сортування тут — повертає "сирий" порядок `find`; споживач
    /// (`BrowserStore`/`BrowserIndex`) сам пересортовує за `sortOrder`.
