...
/// v0.11.0 (P4): банер над таблицею — знайдено незавершену операцію (крах/вимкнення/kill
/// посеред перенесення, або «скопійовано, але не видалено»). Показує перший запис журналу;
/// «Продовжити» ставить решту в чергу (і довидаляє після повторної перевірки), «Відхилити» —
/// прибирає запис. Копії на Mac у tmp-теках не чіпаються — їх прибере sweep, як і раніше.
