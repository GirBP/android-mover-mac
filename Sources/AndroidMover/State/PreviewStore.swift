...
/// Швидкий перегляд (Quick Look) і кеші, з яких він і мініатюри годуються. Тримає сильні
/// однонапрямні посилання на DeviceStore/BrowserStore для client/serial/entries/navigate.
@MainActor
@Observable
final class PreviewStore {
    ...
    /// Ідентичність запиту — токен, а не ім'я файла. Скасований або витіснений запит бачить
    /// чужий токен і не чіпає стан новішого.
    ...
    // Мініатюри з preview-кешу, нуль додаткового трафіку.
    let thumbnails = ThumbnailCache()

    /// Куди drag&drop у Finder лінивo стягує файл рівно в момент дропу. Окремо від
    ...
    /// nonisolated: RemoteFileTransfer.export виконується поза MainActor (Transferable
    ...
    init(deviceStore: DeviceStore, browserStore: BrowserStore) {
        ...
        // Перші 64 КБ фото з телефона для вбудованої мініатюри (ThumbnailCache+Remote).
    ...
    /// Кеш превʼю і кеш перетягувань живуть лише в межах одного запуску — викликається
    ...
    func previewFile(_ entry: RemoteEntry) {
        ...
        // Ключ версії файла (той самий, що для мініатюр): шлях+розмір+mtime відкривається
        // з кешу миттєво.
        let target = Self.previewCacheURL(for: entry)
        if FileManager.default.fileExists(atPath: target.path) {
            previewURL = target
            // ThumbnailCache не питає диск сам — тут єдине джерело правди "цей ключ
            // реально має файл на диску".
        ...
        Task {
            do {
                ...
                if FileManager.default.fileExists(atPath: target.path) {
                    ...
                    // без очікування наступного .task рядка.
                    thumbnails.markCached(Self.previewCacheKey(for: entry))
                    thumbnails.requestThumbnail(for: entry)
                } else {
    ...
    /// ThumbnailCache — щоб знати, який файл уже стягнутий і з нього можна зробити
    ...
    /// Та сама ескалація SIGTERM → 3 с → SIGKILL, що й усюди — голий `terminate()` лишав
    /// би adb, який проігнорував SIGTERM, жити до виходу з додатка.
    func cancelPreview() {
        previewToken = nil
        previewLoadingName = nil
